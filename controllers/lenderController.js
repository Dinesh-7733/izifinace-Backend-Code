...
exports.getAllCustomersForLender = async (req, res) => {
  try {
    ...
    // 1️⃣ Customers registered directly by lender
    const directCustomers = await Customer.find({ 
      registeredBy: lenderId, 
      registeredByModel: "User" 
    });

    // 2️⃣ Agents of lender
    const agents = await AgentModel.find({ lenderId });
    const agentIds = agents.map(agent => agent._id);

    // 3️⃣ Customers registered by lender's agents
    const agentCustomers = await Customer.find({ 
      registeredBy: { $in: agentIds }, 
      registeredByModel: "Agent" 
    });

    // Combine
    const allCustomers = [...directCustomers, ...agentCustomers];
